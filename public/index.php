...
$app->post('/user/register', function (Request $request, Response $response) use ($servername, $username, $password, $dbname) {
    ...
    try {
        ...
        if ($count > 0) {
            $response->getBody()->write(json_encode(["status" => "fail", "token" => null, "data" => ["title" => " Boss the Username already exists."]]));
        } else {
            $hashedPassword = password_hash($pass, PASSWORD_BCRYPT);
            $sql = "INSERT INTO users (username, password) VALUES (:uname, :pass)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([':uname' => $uname, ':pass' => $hashedPassword]);
            $response->getBody()->write(json_encode(["status" => "success", "token" => null, "data" => null]));
        }

    } catch (PDOException $e) {
        $response->getBody()->write(json_encode(["status" => "fail", "token" => null, "data" => ["title" => $e->getMessage()]]));
    }
...
$app->post('/user/login', function (Request $request, Response $response, array $args) use ($servername, $username, $password, $dbname) {
    ...
    try {    
        ...
        if ($userData && password_verify($pass, $userData['password'])) {
            ...
            $response->getBody()->write(json_encode(["status" => "fail", "token" => null, "data" => ["title" => "Authentication Failed"]]));
        }
...
function validateToken($request, $response, $next) {
    ...
    if (!isset($data['token'])) {
        return $response->withStatus(401)->write(json_encode(["status" => "fail", "token" => null, "message" => "Token missing"]));
    }
    ...
    try {
        ...
        if ($decoded->exp < time()) {
            return $response->withStatus(401)->write(json_encode(["status" => "fail", "token" => null, "message" => "Token expired"]));
        }
        ...
        if (!$tokenData || $tokenData['used'] == 1) {
            return $response->withStatus(401)->write(json_encode(["status" => "fail", "token" => null, "message" => "Token already used or invalid"]));
        }

    } catch (Exception $e) {
        return $response->withStatus(401)->write(json_encode(["status" => "fail", "token" => null, "message" => "Unauthorized"]));
    }
...
$app->post('/authors', function (Request $request, Response $response) {
    ...
    $response->getBody()->write(json_encode(["status" => "success", "token" => $response->getHeader('New-Access-Token')[0], "data" => null]));
    return $response;
})->add('validateToken');
